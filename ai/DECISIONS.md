...
| 2026-08-21 | Rename the project to Archon; one `archon` binary | 'Fleet' was always temporary. Naming pass evaluated Westworld (Delos collides with Meta's control-plane storage; Rehoboam too long to type), torus (burned: CoreOS storage + crates.io taken), harbormaster (existing lease tools), and StarCraft names. Archon won: the magistrate who assigned resources, pronounceable, six letters, clean infra collisions, and SC lore of two merging into one matches control-plane-plus-agents. One binary with role subcommands beats separate daemons at this scale; the agent protocol seam makes a future split mechanical. |
