...
| 2026-08-22 | Endpoint generations enforced in production agents; snapshots are one atomic envelope | The review proved production accepted stale session/fence teardown and unauthenticated activation. Agents now reject stale sessions and regressing fences per binding while accepting unseen bindings at the current session (reconcile legitimately activates bindings prepared under a previous process) — matching simulator semantics. Snapshots are a single versioned envelope published fsync+rename before log truncation, with the compaction trigger counted by the persistence sink rather than the cluster's in-memory log (which no longer serializes). Machine-local multi-need requests pick one anchor machine for all needs; topology indices are validated once at placement entry since mid-placement trials legitimately see partial need sets. Teardown acknowledges only after confirmed engine cleanup. Frame length prefixes are refused pre-auth above 16 MiB and streams carry 60 s deadlines. |\n| 2026-08-22 | Device identity lives in graph attrs; claims are the exclusivity contract | Devices declare as capacity-1 nodes with a `dev` attr via ARCHON_DEVICES rather than new kernel types — placement, fencing, and refusal reuse existing machinery unchanged. Activation resolves claimed device nodes to host paths so agents never see the graph. Maintenance drains admission each tick because freed capacity (not just submit time) must re-open scheduling. |\n| 2026-08-22 | Adapter-owned container lifecycle; delegated-scope agents on desktop | Finished containers stay until teardown because a removed container's exit code is unreadable and natural completion would misreport as failure. Stale lease cgroups are reused (kill stragglers) since fresh controllers restart ids at 1. Desktop agents must run under systemd-run --user --scope -p Delegate=true: the kernel denies migrating session-scoped children into the user@1000.service subtree. Podman engines get :Z volume relabeling for SELinux. |\n| 2026-08-22 | Completion is kernel state; drain rides teardown effects; restarts carry lineage | Batch exits become CompleteLease commands (zero completes, else fails) so completion survives replay like every other decision; capacity frees only when enforcement acknowledges release, same wind-down as expiry. Drain applies to both Release and Fence because revocation is an authoritative teardown of the current generation — stale generations are already rejected by session checks upstream. Restart caps/backoff key on the original request via a lineage map: per-generation ids silently made the old cap unenforceable. Container tests skip rootless podman behind the docker CLI (bind mounts read-only under its user mapping). |\n| 2026-08-22 | Snapshots serialize whole state; tuple-keyed maps travel as pairs | Compaction serializes the entire Cluster (opt-in serde on Cluster/Graph) rather than synthesizing baseline commands — simpler and replay-after-restore only handles post-snapshot deltas. JSON cannot use tuple keys in objects, so last_fence's (ProviderId, NodeId) map dehydrates to a vector of pairs via a small serde helper. Controller-side state that outlives restarts (lease commands/images/original requests/restart bookkeeping) rides in the same snapshot since the kernel log never carried it. |
